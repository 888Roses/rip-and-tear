...
public class WarperItem extends Item {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (world instanceof ServerWorld serverWorld) {
            var w = ProjectileEntity.spawn(new WarperEntity(ModEntities.WARPER, world), serverWorld, itemStack, projectile -> {
                projectile.setOwner(user);
                projectile.setPos(user.getPos().getX(), user.getEyeY(), user.getPos().getZ());
                projectile.setVelocity(projectile.getOwner(), Objects.requireNonNull(projectile.getOwner()).getPitch(), projectile.getOwner().getYaw(), 0.0F, 3F, 0F);
            });
